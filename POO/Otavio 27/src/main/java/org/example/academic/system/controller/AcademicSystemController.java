...
/**
 * Camada de coordenação (TUS-2370, TUS-2400).
 * Apenas delega às services e aplica as regras de autorização; não contém
 * lógica de negócio acadêmica (TUS-2400 AC: business logic must not remain
...
public class AcademicSystemController {
    ...
    public void saveData(String fileName) {
        ...
        repo.persistAll(classService.listAllClasses(), fileName);
    ...
    public Optional<AcademicClass> findClassByCode(String code) {
        return classService.findByCode(code);
    }

    public List<AcademicClass> listClasses() {
        return classService.listAllClasses();
    }
    ...
    public String generateClassSummary(String classCode) {
        Role role = authService.getCurrentUser() != null
                ? authService.getCurrentUser().getRole()
                : null;
        return classService.findByCode(classCode)
